...
data Measurement = PoM Rational Rational -- Using Rationals Here is probably meh
...
pow (PoM v1 u1) p
    =  ( PoM (toRational (fromRational v1 ** p))
                           ( (toRational (fromRational v1 ** p)) * (toRational p) * (u1 / v1) )
                       )

getTPrime :: Measurement -> Measurement -> Double
getTPrime (PoM v1 u1) (PoM v2 u2)
    = abs . fromRational $ ( (v1 - v2)/rootSquares [u1, u2] )
...
helpMessage :: String
helpMessage =
  "Format a Measurement like this: (2.0 `PoM` 0.05)\n" ++
  "You can add, substract, multiply and divide as normal.\n" ++
  "You can raise a Measurement to an Integer power by calling: pow (Measurement) n\n" ++
  ...
  "Determine the t' value between two measurements by calling: getTPrime m1 m2\n" ++
  "(Remember that less than 1 is agreement and greater than 3 is disagreement).\n"
